...
const Modal = ({ children, onClose }: { children: React.ReactNode, onClose?: ([...args]?: any[]) => void }) => {
    ...
    return (
        <animated.div
            style={slideIn}
            className="fixed flex items-center justify-center bg-black/50 top-0 left-0 right-0 bottom-0 z-[60]"
            onClick={e => overlayClose(e)}
            id="overlay"
        >
            <div className="bg-orange-800 border-r-4 border-b-4 border-t-[1px] border-l-[1px] border-black w-[90%] md:w-[50%] rounded-lg px-2 py-4 fixed z-[70] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
